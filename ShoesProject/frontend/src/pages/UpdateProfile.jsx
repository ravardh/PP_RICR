...
export default function UpdateProfile() {
  ...
  const validate = () => {
    ...
    if (!nameRegex.test(inputData.fullName))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update session storage after updating profile
      ...
      }, 1500); // Allow toast to be visible before reload
    } catch (error) {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex flex-col items-center gap-2">
            <img
              src={inputData.image || pp}
              alt="Profile"
              className="w-40 h-40 border-3 border-gray-800 rounded-full p-1 object-cover"
            />
